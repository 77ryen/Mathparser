...
REAL _find_roots_simple(REAL_FUNCTION funct,
    ...
    throw std::logic_error("Number of iterations exceeded without achieving desired precision. Maybe too few iterations or the function does not have roots?");

}

REAL compute_integral(REAL_FUNCTION funct, REAL a, REAL b, long long number_of_divisions, bool parallel){

    if(number_of_divisions <= 0){
        throw std::range_error("The number of divisions must be a non-zero positive integer.");
    }

    REAL h = (b - a) / static_cast<REAL>(number_of_divisions);

    REAL sum = 0;

    sum += (funct(b) + funct(a))/2;

    #pragma omp parallel for if(parallel) reduction(+:sum)
    for(long long i = 1; i < number_of_divisions; i++){
